...
let cached=JSON.stringify({...boot,build:'OLD'}),builds=0;
c.BUILD_ID='NEW';
c.CacheService.getScriptCache=()=>({get:()=>cached,put:(key,value)=>{cached=value}});
const buildBoot=c.buildBoot_;c.buildBoot_=()=>{builds++;return buildBoot()};
assert.equal(c.bootstrap().build,'NEW','old deployment cache cannot report a false old-page warning');
assert.equal(builds,1,'deployment change rebuilds bootstrap data');
assert.equal(c.bootstrap().build,'NEW');
assert.equal(builds,1,'matching deployment still reuses bootstrap cache');
console.log('16 startup/settings backend assertions passed');
