...
const before=reads,summary=context.getNavigationSummary();
assert.equal(reads-before,1,'delivery and dashboard share one full-history read');
assert.deepEqual(JSON.parse(JSON.stringify(summary.pending)),JSON.parse(JSON.stringify(result.pending)));
assert.deepEqual(JSON.parse(JSON.stringify(summary.dashboard)),JSON.parse(JSON.stringify(result.dashboard)));
console.log('21 backend navigation assertions passed; summary split preserves all pending orders and totals');
