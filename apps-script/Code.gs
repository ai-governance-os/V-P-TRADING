...
//
// 两种用法：
//  1) 直接打开部署网址 → 走旧版内置 HTML 界面（index.html），保留作后备/快速用。
//  2) 加 ?fn=函数名&args=[JSON数组] 请求 → 走 JSON API，给 Vercel 上的新界面用。
//     写操作（saveOrder 等）请用 POST，Content-Type: text/plain，body 是
//     JSON 字符串 {"fn":"saveOrder","args":[...]}（用 text/plain 是为了不触发
//     浏览器的 CORS 预检请求，Apps Script 不处理 OPTIONS 请求）。

function doGet(e) {
  if (e && e.parameter && e.parameter.fn) {
    return apiDispatch_(e.parameter.fn, parseApiArgs_(e.parameter.args));
  }
...
function doPost(e) {
  ...
  try {
    ...
    return apiError_('请求体不是合法 JSON');
  }
  return apiDispatch_(body.fn, body.args || []);
}
...
function apiDispatch_(fnName, args) {
  var fn = API_FNS[fnName];
  var out;
  if (!fn) {
    out = { error: 'Unknown function: ' + fnName };
  } else {
    try {
